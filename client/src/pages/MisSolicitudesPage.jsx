    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-gray-800">
          {isElderlyOrTutor ? 'Mis Solicitudes' : 'Mis Tareas Aceptadas'}
        </h1>
        {isElderlyOrTutor && (
          <Link
            to="/solicitudes/nueva"
            className="bg-uct-gold text-uct-blue-dark px-5 py-2.5 rounded-xl font-semibold hover:bg-uct-gold-light shadow"
          >
            ➕ Nueva
          </Link>
        )}
      ...
      <div className="flex gap-2 mb-6 flex-wrap">
        {['', 'PENDIENTE', 'EN_CURSO', 'COMPLETADA', 'FINALIZADA'].map(
          (estado) => (
            <button
              key={estado}
        ...
        <div className="space-y-3">
          {filtered.map((sol) => (
            ...
            >
              ...
              <span
                className={`${estadoStyles[sol.estado]} px-4 py-1.5 rounded-full text-xs font-bold ml-4`}
              >
                {sol.estado}
              </span>
            </Link>
