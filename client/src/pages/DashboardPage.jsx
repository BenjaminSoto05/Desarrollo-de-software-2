  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="px-6 py-8 mb-6 text-center bg-white rounded-2xl shadow-lg border-b-4 border-[#0d6efd]">
        <h1 className="text-3xl font-bold text-[#0d6efd] mb-2">
          ¡Hola, {user?.nombre}! 👋
        </h1>
        <p className="text-gray-500 text-lg">
          {isElderlyOrTutor
            ? 'Gestiona tus solicitudes de ayuda y conecta con voluntarios.'
            : 'Encuentra solicitudes de ayuda y marca la diferencia.'}
        </p>
        <div className="flex gap-3 justify-center mt-5">
          {isElderlyOrTutor && (
            <Link
              to="/solicitudes/nueva"
              className="bg-[#198754] hover:bg-[#157347] text-white px-6 py-2.5 rounded-full font-medium transition-all btn-custom shadow-sm"
            >
              + Nueva Solicitud
            </Link>
          )}
          <Link
            to="/solicitudes"
            className="bg-[#343a40] hover:bg-[#23272b] text-white px-6 py-2.5 rounded-full font-medium transition-all btn-custom shadow-sm"
          >
            Ver Solicitudes
          </Link>
        </div>
        ...
        {[
          {
            label: 'Total',
            value: stats.total,
            bg: 'bg-[#0d6efd]/10 border-[#0d6efd]/20 text-[#0d6efd]',
          },
          {
            label: 'Disponibles',
            value: stats.pendientes,
            bg: 'bg-[#198754]/10 border-[#198754]/20 text-[#198754]',
          },
          {
            label: 'En Curso',
            value: stats.enCurso,
            bg: 'bg-[#ffc107]/10 border-[#ffc107]/30 text-[#856404]',
          },
          {
            label: 'Finalizadas',
          ...
          <div
            key={s.label}
            className={`${s.bg} border rounded-2xl p-5 card-hover`}
          >
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-gray-400 mt-0.5">
                    {sol.categoria?.nombre} · {sol.comuna}
                  </p>
                </div>
                <span
                  className={`${estadoBadges[sol.estado]} px-3 py-1 rounded text-xs font-medium`}
                >
                  {sol.estado}
                </span>
              </Link>
