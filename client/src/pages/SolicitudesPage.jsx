    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Solicitudes disponibles actualmente:{' '}
        ...
        {['ADULTO_MAYOR', 'TUTOR'].includes(user?.rol) && (
          <Link
            ...
            className="bg-[#198754] hover:bg-[#157347] text-white px-5 py-2.5 rounded-lg font-medium transition-all shadow-sm"
          >
            + Nueva Solicitud
          </Link>
        )}
      ...
      <div className="bg-white rounded-2xl shadow-sm border p-4 mb-6 flex flex-wrap gap-3">
        <select
          ...
          className="px-3 py-2 border rounded-xl text-sm bg-gray-50"
        ...
        <input
          ...
          className="px-3 py-2 border rounded-xl text-sm bg-gray-50 w-48"
      ...
      {loading ? (
        <div className="text-center py-16">
          <div className="animate-spin h-10 w-10 border-4 border-uct-blue border-t-transparent rounded-full mx-auto"></div>
        </div>
            ...
            >
              <div className="p-5">
                <div className="flex items-start justify-between mb-3">
                  <span className="bg-uct-blue/10 text-uct-blue px-3 py-1 rounded-full text-xs font-semibold">
                    {sol.categoria?.nombre}
                  </span>
                  <span
                    className={`${estadoStyles[sol.estado]} px-3 py-1 rounded-full text-xs font-semibold`}
                  >
                    {sol.estado}
                  </span>
                </div>
                <h3 className="font-bold text-gray-800 mb-2 line-clamp-2 group-hover:text-uct-blue transition-colors">
                  {sol.titulo}
                </h3>
              ...
              <div className="border-t px-5 py-3 flex gap-2">
                <Link
                  ...
                  className="flex-1 text-center text-sm text-uct-blue font-semibold py-2 rounded-lg hover:bg-uct-blue/5 transition-colors"
                >
                  Ver Detalle
                </Link>
                {user?.rol === 'ESTUDIANTE' && sol.estado === 'PENDIENTE' && (
                  <button
                    ...
                    className="flex-1 text-center text-sm bg-uct-blue text-white font-semibold py-2 rounded-lg hover:bg-uct-blue-light transition-colors"
                  >
                    Aceptar
                  </button>
                )}
      ...
      {pagination.totalPages > 1 && (
        <div className="flex justify-center gap-2 mt-8">
          {Array.from({ length: pagination.totalPages }, (_, i) => (
            <button
              key={i + 1}
              onClick={() => fetchSolicitudes(i + 1)}
              className={`w-10 h-10 rounded-xl text-sm font-semibold transition-all ${
                pagination.page === i + 1
                  ? 'bg-uct-blue text-white shadow'
