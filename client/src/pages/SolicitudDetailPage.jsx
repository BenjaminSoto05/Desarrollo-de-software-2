...
export default function SolicitudDetailPage() {
  ...
  if (loading)
    return (
      <div className="flex justify-center py-20">
        <div className="animate-spin h-10 w-10 border-4 border-uct-blue border-t-transparent rounded-full"></div>
      </div>
    );
  if (!sol) return null;
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8">
      <button
        onClick={() => navigate(-1)}
        className="text-uct-blue font-semibold mb-4 hover:underline"
      >
        ← Volver
      </button>
      <div className="bg-white rounded-2xl shadow-sm border overflow-hidden">
        ...
        <div className="bg-gradient-to-r from-uct-blue to-uct-blue-light p-6 text-white">
          <div className="flex items-start justify-between">
            ...
            <span
              className={`${estadoStyles[sol.estado]} px-4 py-1.5 rounded-full text-sm font-bold`}
            >
              {sol.estado}
            </span>
          </div>
        ...
        <div className="p-6 space-y-6">
          ...
          <div>
            ...
            <p className="text-gray-600">{sol.descripcion}</p>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 bg-gray-50 rounded-xl p-4">
            <div>
              ...
              <p className="font-semibold">
                {new Date(sol.fechaProgramada).toLocaleDateString('es-CL')}
              </p>
            </div>
            <div>
              <p className="text-xs text-gray-400">Hora</p>
              <p className="font-semibold">{sol.horaProgramada}</p>
            </div>
            <div>
              <p className="text-xs text-gray-400">Comuna</p>
              <p className="font-semibold">{sol.comuna}</p>
            </div>
            <div>
              <p className="text-xs text-gray-400">Dirección</p>
              <p className="font-semibold">{sol.direccion}</p>
            </div>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            ...
            {sol.voluntario && (
              <div className="bg-blue-50 rounded-xl p-4 border border-blue-100">
                ...
                <p className="font-bold text-gray-800">
                  {sol.voluntario.nombre} {sol.voluntario.apellido}
                </p>
          ...
          <div className="flex flex-wrap gap-3">
            {user?.rol === 'ESTUDIANTE' && sol.estado === 'PENDIENTE' && (
              <button
                onClick={() =>
                  handleAction(
                    () => solicitudesAPI.accept(id),
                    '¿Aceptar esta solicitud?'
                  )
                }
                className="bg-uct-blue text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-uct-blue-light transition-all"
              >
                🤝 Aceptar
              </button>
            )}
            {esVoluntario && sol.estado === 'EN_CURSO' && (
              <>
                ...
                <button
                  onClick={() =>
                    handleAction(
                      () => solicitudesAPI.cancelAcceptance(id),
                      '¿Cancelar tu aceptación?'
                    )
                  }
                  className="bg-red-100 text-red-700 px-6 py-2.5 rounded-xl font-semibold hover:bg-red-200 transition-all"
                >
                  ❌ Cancelar Tarea
                </button>
              </>
            ...
            {esSolicitante && sol.estado === 'COMPLETADA' && (
              <button
                onClick={() =>
                  handleAction(
                    () => solicitudesAPI.confirm(id),
                    '¿Confirmar que la tarea fue completada?'
                  )
                }
                className="bg-uct-gold text-uct-blue-dark px-6 py-2.5 rounded-xl font-semibold hover:bg-uct-gold-light transition-all shadow"
              >
                🏆 Confirmar Finalización
              </button>
            )}
            {(esSolicitante || esVoluntario) &&
              (sol.estado === 'FINALIZADA' || sol.estado === 'COMPLETADA') && (
                <button
                  onClick={() => setShowEvalForm(!showEvalForm)}
                  className="bg-purple-100 text-purple-700 px-6 py-2.5 rounded-xl font-semibold hover:bg-purple-200 transition-all"
                >
                  ⭐ Evaluar
                </button>
              )}
          ...
          {showEvalForm && (
            <form
              onSubmit={handleEval}
              className="bg-purple-50 rounded-xl p-5 border border-purple-100 space-y-4"
            >
              ...
              <div className="flex gap-2">
                {[1, 2, 3, 4, 5].map((n) => (
                  <button
                    key={n}
                    type="button"
                    onClick={() => setEvalForm({ ...evalForm, puntuacion: n })}
                    className={`text-3xl transition-transform ${evalForm.puntuacion >= n ? 'scale-110' : 'opacity-30'}`}
                  >
                    ⭐
                  </button>
                ))}
              ...
              <textarea
                ...
                onChange={(e) =>
                  setEvalForm({ ...evalForm, comentario: e.target.value })
                }
              ...
              <div className="space-y-3">
                {sol.evaluaciones.map((ev) => (
                  <div key={ev.id} className="bg-gray-50 rounded-xl p-4">
                    ...
                    {ev.comentario && (
                      <p className="text-sm text-gray-600">{ev.comentario}</p>
                    )}
                  </div>
                ))}
              </div>
