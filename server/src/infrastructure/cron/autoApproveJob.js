...
function startAutoApproveJob(autoApproveUseCase) {
  ...
  cron.schedule('0 * * * *', async () => {
    logger.info('⏰ Ejecutando auto-aprobación de solicitudes (RN-12)...', {
      module: 'cron',
    });
    try {
      ...
      if (result.solicitudesAprobadas > 0) {
        logger.info(
          `✅ Auto-aprobadas: ${result.solicitudesAprobadas} solicitudes`,
          { module: 'cron' }
        );
      } else {
        logger.info('✅ Sin solicitudes pendientes de auto-aprobación', {
          module: 'cron',
        });
      }
    } catch (error) {
      logger.error(`❌ Error en auto-aprobación: ${error.message}`, {
        module: 'cron',
      });
    }
  });

  logger.info('📅 Cron job de auto-aprobación iniciado (cada hora)', {
    module: 'cron',
  });
}
